...
function unselectRows(rowsArray) {
    for (var i = 0; i < rowsArray.length; i++) {
        rowsArray[i].className = "draggable_notselected";
    }
}
...
function dragOver(event) {
    // In js esistono dei comportamenti di default. In questo caso il comportamento di defalut è quello di NON chiamare il drop event
    ...
    dest.className = "draggable_selected";
}
...
function dragLeave(event) {
    ...
    dest.className = "draggable_notselected";
}
...
function drop(event) {
    ...
    //The Array.from functions puts all the elements into an array
    ...
    if (rowsArray.indexOf(startElement) < indexDest)
        ...
        //Js doesn't have an insertAfter funcion
